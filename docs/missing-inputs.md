...
| Gallery images | — (asset) | `/gallery` | Not supplied; the page renders a labelled empty state until images are uploaded to the `gallery` bucket from `/admin/media` (Admin-only). Caption defaults to "Safeway Tyre" when a Media record supplies none |
